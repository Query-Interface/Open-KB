...
export const EditBoard: React.FC = () => {
    ...
    return <Collapse defaultActiveKey={['1', '2']} expandIconPosition="right">
        <Panel header={<div className="edit-panel-section-header">Board title</div>} key="1">
            <EditArea content={board?.title??''} placeholder={'Set a title'} key={`title-${board?.id}`} saveCallback={onSaveTitle}/>
        </Panel>
        <Panel header={<div className="edit-panel-section-header">Board description</div>} key="2">
            <EditArea content={board?.description??""} placeholder={'Set a description'} key={`desc-${board?.id}`} saveCallback={onSaveDescription} />
        </Panel>
    </Collapse>;
};
