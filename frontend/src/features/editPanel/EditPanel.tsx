...
export const EditPanel: React.FC = () => {
  ...
  return (
    <Sider width={400} trigger={null} collapsedWidth={0} collapsible collapsed={editPanelVisible}>
      <div className="edit-panel-header">
        <span>{getPanelTitle()}</span>
        <div className="btn btn-close-panel">
          <CloseOutlined onClick={(event): void => onCancel(event)} />
        </div>
      </div>
      <div className="edit-panel">{renderEditPanelContent()}</div>
    </Sider>
  );
};
