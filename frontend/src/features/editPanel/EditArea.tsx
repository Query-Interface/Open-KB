...
export const EditArea: React.FC<EditAreaProps> = ({ content, placeholder, saveCallback }: EditAreaProps) => {
  ...
  if (!editMode) {
    if (text) {
      elements.push(
        <div style={{ width: '100%' }} onClick={(event): void => edit(event)} key="contentEdited">
          {text}
        </div>,
      );
    } else {
      elements.push(
        <div style={{ width: '100%', opacity: '0.5' }} onClick={(event): void => edit(event)} key="contentEdited">
          {placeholder}
        </div>,
      );
    }
    ...
    elements.push(
      <Button className="btn-section-panel" type="primary" onClick={(event): void => save(event)} key="bt-save">
        Save
      </Button>,
    );
    elements.push(
      <Button
        className="btn-section-panel"
        type="default"
        shape="circle"
        key="bt-cancel"
        icon={<CloseOutlined />}
