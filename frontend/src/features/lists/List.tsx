...
export const List: React.FC<ListProps> = ({ boardId, listId, index }: ListProps) => {
        ...
        {(provided: DroppableProvided): React.ReactElement => (
          <div className="list-cards" ref={provided.innerRef} {...provided.droppableProps}>
            {cards.map(function (card: ModelCard, index: number) {
              return (
                <Card card={Object.assign({}, card, { parentList: listId })} index={index} key={`card-${card.id}`} />
              );
            })}
  ...
  const renderColumnWithDnD = (list: ModelList): React.ReactElement => {
    return (
      <Draggable draggableId={`list-drag-${list.id}`} key={list.id} index={index}>
          ...
          <div className="list-column" ref={provided.innerRef} {...provided.draggableProps}>
            <div className="list-container">
              <div className="list-header" {...provided.dragHandleProps}>
                ...
                <div className="btn btn-list-menu">
                  <span>
                    <EllipsisOutlined />
                  </span>
                </div>
                <div className="btn btn-list-add" onClick={(event): void => onAddCard(event)}>
                  <span>
                    <PlusOutlined />
                  </span>
                </div>
              </div>
              <div className="list-content">{renderCardsWithDnd(list.cards || [])}</div>
            </div>
          </div>
        )}
      </Draggable>
