...
const HomePage: React.FC = () => {
    ...
    if (boards && boards.length > 0) {
        return <div className='home-board-container'>
            <ul className='home-board-list'>
                {boards.map( board => <BoardButton {...board} key={board.id} />)}
                <li className='home-board-item' onClick={(event: React.MouseEvent): void => { createBoard(event); }}>
                    <div className='home-board-item-add'><PlusOutlined /> Create a board</div>
                </li>
            </ul>
        </div>;
    } else {
...
const BoardButton: React.FC<BoardDetails> = ({id, title}: BoardDetails) => {
    ...
    return <li onClick={(event): void => selectBoard(event, id)}>
